...
start KF-WestLondon.rom?Listen?Game=ScrnBalanceSrv.ScrnGameType?Difficulty=7?GameLength=2?VACSecured=true?MaxPlayers=6
...
KF-WestLondon.rom?Listen?Game=ScrnBalanceSrv.ScrnGameType?Difficulty=7?GameLength=2?VACSecured=true?MaxPlayers=6
...
4.  Select Map. Game length selection is not available there, but we will fix
    it later.
5.  Go to "Server Rules" page.
6.  Check "View Advanced Options"
7.  If you want to play Solo, then set Max Players = 1 and Max Spectators = 0.
    This will emulate Solo mode, but won't crash your game on some maps.
8.  Go to "Sandbox" page. We won't use Sandbox mode. It is just a workaround
    how to set game length in custom game modes (ScrN Floor).
9.  Select Difficulty and Game Length (do NOT use Custom!).
10. Click "LISTEN" button without leaving "Sandbox" page (or length will reset)
...
ucc server KF-WestLondon.rom?Game=ScrnBalanceSrv.ScrnGameType?Difficulty=7?GameLength=2?VACSecured=true?MaxPlayers=12?Port=7707 log=KFServer.log
